...
def design_fir_filter(fir_length, iir_filters, sr):
...
def evaluate_performance(fir_coeffs, iir_filters, sr):
    """フィルタ性能の評価"""

    nyquist = sr / 2

    # ITU-T G.227の理論特性
    f_m = np.linspace(0, nyquist, 2**15)
    p = 1j * f_m / 1000
    numerator = 18400 + 91238*p**2 + 11638*p**4 + p*(67280 + 54050*p**2)
    denominator = 400 + 4001*p**2 + p**4 + p*(36040 + 130*p**2)
    loss_m = np.abs(numerator / denominator)
    ...
    # 周波数帯域別評価（サンプルレートに応じて調整）
    freq_bands = [(10, 100), (100, 1000), (1000, 10000), (10000, nyquist)]
    band_names = ['10-100Hz', '100-1kHz', '1k-10kHz', f'10k-{int(nyquist/1000)}kHz']
    band_rmse = []
...
def load_current_coefficients(sr):
    """現在の係数を読み込み"""
    ...
    try:
        ...
        sr_key = str(sr)
        if sr_key not in coeffs:
            print(f"警告: サンプルレート {sr} の係数が見つかりません")
            return None, None

        current_fir = coeffs[sr_key]['fir']
        current_length = len(current_fir)

        return current_fir, current_length
    except FileNotFoundError:
        print(f"警告: {COEFFS_PATH} が見つかりません")
        return None, None


def analyze_coefficients(sr, test_lengths=None, target_rmse=0.003, target_max_error=0.02):
    """係数分析のメイン関数"""

    print(f"\n{'='*60}")
    print(f"=== FIR係数最適化分析 (サンプルレート: {sr}Hz) ===")
    print(f"{'='*60}")

    # 現在の係数を読み込み
    current_fir, current_length = load_current_coefficients(sr)
    ...
    # IIRフィルタの読み込み
    iir_filters = load_iir_filters(sr)
    ...
    # 現在の係数の性能評価
    current_performance = evaluate_performance(current_fir, iir_filters, sr)
    results.append({
    ...
    for length in test_lengths:
        ...
        # 新しくFIRフィルタを設計
        fir_coeffs = design_fir_filter(length, iir_filters, sr)

        # 性能評価
        performance = evaluate_performance(fir_coeffs, iir_filters, sr)
...
def visualize_results(results, current_length, sr, save_filename):
    """結果の可視化"""

    nyquist = sr / 2
    fig, axes = plt.subplots(3, 3, figsize=(16, 12))
    fig.suptitle(f'FIR係数最適化分析 (サンプルレート: {sr}Hz)', fontsize=14)
    ...
    ax.bar(range(len(lengths)), mult_per_block, color=colors, alpha=0.7)
    ax.set_xlabel('係数数')
    ...
    ax.scatter(reduction_percent, rmse_values, s=100, c=['green' if ic else 'blue' for ic in is_current],
    ...
    band_names = current_result['performance']['band_names']
    ...
    print(f"グラフが保存されました: {save_filename}")
...
def main():
    ...
    for sr in sample_rates:
        ...
        if results:
            ...
            # 結果の可視化
            output_filename = os.path.join(OUTPUT_DIR, f'fir_optimization_analysis_{sr}.png')
            visualize_results(results, current_length, sr, output_filename)
